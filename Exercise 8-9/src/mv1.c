...
void mv(base_t **A, int nrows, int ncols, int nrows_a_loc, int ncols_a_loc,
    ...
    MPI_Allgatherv(&x[0], nrows_x_loc, MPI_DOUBLE, &receiveCount, xRowsCount, relativeOffset, MPI_DOUBLE, MPI_COMM_WORLD);
    for (int i = 0; i < nrows_a_loc; i++) {
        b[i] = 0;
        for (int j = 0; j < ncols_a_loc; j++) {
            b[i] += A[i][j] * receiveCount[j];
        }
    }
}
